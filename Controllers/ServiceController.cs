...
namespace IlusalongAPI.Controllers
{
    ...
    public class ServiceController : ControllerBase
    {
        ...
        public IActionResult AddService([FromBody] Service service)
        {
            ...
            var category = _context.Categories.Find(service.CategoryId);
            if (category == null)
                return BadRequest("Категория с указанным ID не найдена.");

            // Добавляем услугу в базу данных, связывая с существующей категорией
            _context.Services.Add(service);
            _context.SaveChanges();  // Сохраняем услугу в базе данных
